...
arneodo_attractor = AttractorConfig(
    ...
    _arneodo,
    params=[
        AttractorParam("$a$", -5.5, -5.83, 2.4, 0.01),
    ...
    description=(
        "The Arneodo attractor is a 3D chaotic system that demonstrates the \
            emergence of 'spiral chaos' through homoclinic loops. Its trajectory \
            typically features a central spiraling core that expands and \
            contracts, creating a layered structure that appears \
            both organic and rigidly geometric."
    ),
    equation_text=r"$\\\dot{x}=y,\\\dot{y}=z,\\\dot{z}=-ax-by-z+cx^3$",
    presets={
        "Classic": {"$a$": -5.5, "$b$": 3.5, "$c$": -1.0},
        "Single plane": {"$a$": -4.34, "$b$": 3.84, "$c$": -2.0},
    },
    prompts=[
        ...
        "Adjust $b$ to control the damping of the vertical oscillations. Higher \
            ...
            leading to a flatter, more disc-like appearance. Lower \
            values allow the 'spiral' to stretch vertically.",
        "Tiny changes to $c$ can cause the attractor to transition between structured \
            ribbons and a dense, cloud-like chaotic state. Watch how it affects the \
